...
class GrainApproval extends TableWidget
{
    use HasPageShield;
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->deferLoading(true)
            ->query(fn (): Builder => Grain::query()->where('status', '!=','disbursed'))
            ->columns([
            ...
            ->recordActions([
                ViewAction::make()
                    ...
                    ->schema(fn($schema) => GrainResource::infolist($schema))
                    ->slideOver(),
            ])
            ->toolbarActions([
                BulkActionGroup::make([
                    BulkAction::make('approval')
                        ...
                        ->modalDescription('Are you sure you want to approve the selected grains?')
                        ...
                        ->action(function (Collection $records) {
                            ...
                            Notification::make()
                                ->title('Approved')
                                ->body('Selected grains approved successfully')
                                ->success()
                                ->send();
                        })
                        ->slideOver()
