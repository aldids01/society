...
class LifelineExporter extends Exporter
{
    ...
    public static function getColumns(): array
    {
        return [
            ...
            ExportColumn::make('llsc')
                ->label('Amount'),
            ExportColumn::make('llsc_gechaan')
                ->label('GECHAAN')
                ->getStateUsing(function ($record) {
                    if ($record->employee->department->category->name === 'GECHAAN') {
                        return number_format($record->llsc, 2);
                    } else {
                        return number_format(0, 2);
                    }
                }),
            ExportColumn::make('llsc_llsc')
                ->label('LLSC')
                ->getStateUsing(function ($record) {
    ...
    {
        $body = 'Your lifeline export has completed and ' . number_format($export->successful_rows) . ' ' . str('row')->plural($export->successful_rows) . ' exported.';
