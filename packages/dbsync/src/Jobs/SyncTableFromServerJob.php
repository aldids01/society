...
namespace Aldids\FilamentDbSync\Jobs;
...
use Aldids\FilamentDbSync\Services\ModelsServices;
...
class SyncTableFromServerJob implements ShouldQueue
{
    ...
    public int $timeout = 0; // maximum number of seconds the job is allowed to run

    public int $tries = 3; // maximum number of attempts

    public int $maxExceptions = 3; // maximum number of exceptions to handle

    public int $backoff = 3; // number of seconds to wait before retrying the job

    protected string $plugin_ids;

    protected string $models;

    protected array $model_definition;

    protected array $models_datas;

    protected mixed $sync_host;

    protected mixed $sync_config;

    protected mixed $sync_token;

    // Used for getting data from the sync host according to the table name
    protected string $api_get_data;
