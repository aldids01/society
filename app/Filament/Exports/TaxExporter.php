...
class TaxExporter extends Exporter
{
    ...
    public static function getColumns(): array
    {
        return [
            ...
            ExportColumn::make('employee.gross')
                ->label('Gross'),
            ExportColumn::make('tax')
                    ->formatStateUsing(fn ($state) => number_format($state, 2)),
            ExportColumn::make('tax_gechaan')
                    ->label('GECHAAN')
                    ->getStateUsing(function ($record) {
    ...
    {
        $body = 'Your tax export has completed and ' . number_format($export->successful_rows) . ' ' . str('row')->plural($export->successful_rows) . ' exported.';
