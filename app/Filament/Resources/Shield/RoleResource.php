                ...
                Tables\Actions\Action::make('user')
                    ...
                    ->form(fn ($record) => [
                        Forms\Components\CheckboxList::make('users')
                            ->label('Select Users') // Provide a label
                            ->searchable()
                            ->options(
                            // Fetch users from the database
                                DB::table('users')->pluck('name', 'id') // Ensure you have 'name' and 'id' columns in the 'users' table
                            )
                            ->default($record->users->pluck('id')->toArray()),
                    ])
                    ->action(function ($record, array $data){
                        // Check if 'roles' is set, if not, use an empty array
                        $userIds = $data['users'] ?? [];

                        // Ensure the role exists and retrieve its ID
                        $roleId = $record->id; // Assuming $record is a Role instance

                        // Prepare an array for syncing
                        $syncData = [];
                        foreach ($userIds as $userId) {
                            $syncData[] = [
                                'role_id' => $roleId,
                                'model_type' => 'App\Models\User', // Adjust if your User model's namespace is different
                                'model_id' => $userId,
                            ];
                        }

                        // Insert or update the records in model_has_roles table
                        DB::table('model_has_roles')->upsert($syncData, ['role_id', 'model_id'], ['role_id']); // Use upsert to prevent duplicates
