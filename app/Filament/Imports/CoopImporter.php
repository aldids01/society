...
class CoopImporter extends Importer
{
    ...
    public static function getColumns(): array
    {
        return [
            ImportColumn::make('employee_id')
                ...
                ->requiredMapping()
                ->rules(['required']),
            ImportColumn::make('lint')
                ->label('Loan Interest')
                ->requiredMapping()
                ->rules(['required']),
            ImportColumn::make('lprin')
                ->label('Loan Principal')
                ->requiredMapping()
                ->rules(['required']),
            ImportColumn::make('gint')
                ->label('Grain Interest')
                ->requiredMapping()
                ->rules(['required']),
            ImportColumn::make('gprin')
                ->label('Grain Principal')
                ->requiredMapping()
                ->rules(['required']),
            ImportColumn::make('saved')
                ->label('Saving')
                ->requiredMapping()
                ->rules(['required']),
        ];
    ...
    {
        $body = 'Your Societies import has completed and ' . number_format($import->successful_rows) . ' ' . str('row')->plural($import->successful_rows) . ' imported.';
