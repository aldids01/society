...
<body>
    <div class="invoice-container">
        <div class="header">
            <h1>GECHAAN STAFF COOPERATIVE AND MULTIPURPOSE SOCIETY <br/>17 Dabon Warwar Street, Taraba State, Sardauna LGA<br/>{{ $record->applicant->name }} Loan Request Form</h1>
        ...
        <table>
            ...
            <tbody>
                @php
                    ...
                    $totalPayment = 0;
                @endphp
                @foreach ($record->loanAmort as $amort)
                <tr>
                    <td>{{ $amort->annual }}</td>
                    <td>{{ $amort->period }}</td>
        ...
        <table>
            <tr>
                <th>Checked By</th>
                <td>{{ $record->approvals->check->name }}</td>
                <th>Checked Date</th>
                <td>{{ $record->approvals->checkeddate->format('F jS, Y g:ia') }}</td>
            </tr>
            <tr>
                <th>Approved By</th>
                <td>{{ $record->approvals->approve->name }}</td>
                <th>Approved Date</th>
                <td>{{ $record->approvals->approveddate->format('F jS, Y g:ia') }}</td>
            </tr>
            <tr>
                <th>Disbursed By</th>
                <td>{{ $record->approvals->disburse->name }}</td>
                <th>Disbursed Date</th>
                <td>{{ $record->approvals->disburseddate->format('F jS, Y g:ia') }}</td>
            </tr>
        </table>
