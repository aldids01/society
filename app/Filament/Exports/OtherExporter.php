...
class OtherExporter extends Exporter
{
    ...
    public static function getColumns(): array
    {
        return [
            ...
            ExportColumn::make('other')
                ->label('Amount'),
            ExportColumn::make('other_gechaan')
                ->label('GECHAAN')
                ->getStateUsing(function ($record) {
                    if ($record->employee->department->category->name === 'GECHAAN') {
                        return number_format($record->other, 2);
                    } else {
                        return number_format(0, 2);
                    }
                }),
            ExportColumn::make('other_llsc')
                ->label('LLSC')
                ->getStateUsing(function ($record) {
    ...
    {
        $body = 'Your other export has completed and ' . number_format($export->successful_rows) . ' ' . str('row')->plural($export->successful_rows) . ' exported.';
